/* ═══════════════════════════════════════════════════════════
   ppl-store.js — profils « pseudo » + progression PPL
...
const PplStore = (() => {
  ...
  function readCount(mat) {
    ...
    const m = PPL.byId(mat); if (!m) return 0;
    let n = 0;
  ...
  function syncCours(mat) {
    const p = current(); if (!p) return;
    const m = PPL.byId(mat); if (!m) return;
  ...
  function freshCount(mat) {
    const p = current(); if (!p) return 0;
    const src = mat ? (PPL.byId(mat)?.quiz || []) : PPL.allQuiz();
  ...
  function globalStats() {
    ...
    return {
      ...
      seen: qs,
      mastered,
      coverage: PPL.quizCount() ? Math.round(qs / PPL.quizCount() * 100) : 0,
      mastery:  PPL.quizCount() ? Math.round(mastered / PPL.quizCount() * 100) : 0,
      cours:    PPL.sectionCount() ? Math.round(sec / PPL.sectionCount() * 100) : 0,
      mnemos:   mnemoCount(),
  ...
  /** Score de préparation 0-100 : cours lus, couverture, maîtrise, régularité. */
  function readiness() {
    const g = globalStats();
    const val = Math.round(g.cours * .28 + g.coverage * .22 + g.mastery * .35 + Math.min(g.streak, 10) * 1.5);
  ...
  function exportJSON() {
    const p = current();
    return JSON.stringify({ app: 'ppl-prep', version: 1, profile: p, exportedAt: new Date().toISOString() }, null, 2);
  }
